...
function SilkPlane({ uniforms }: SilkPlaneProps) {
  ...
  useLayoutEffect(() => { meshRef.current?.scale.set(viewport.width, viewport.height, 1); }, [viewport]);
  useFrame((_, delta) => { uniforms.uTime.value = Number(uniforms.uTime.value) + 0.1 * delta; });
  return <mesh ref={meshRef}><planeGeometry args={[1, 1]} /><shaderMaterial uniforms={uniforms} vertexShader={vertexShader} fragmentShader={fragmentShader} /></mesh>;
}
...
export default function Silk({ speed = 5, scale = 1, color = "#7C3AED", noiseIntensity = 1.5, rotation = 0, className }: SilkProps) {
  ...
  return <div className={`silk-container ${className ?? ""}`}><Canvas dpr={[1, 1.5]} frameloop="always" gl={{ alpha: true, antialias: true }}><SilkPlane uniforms={uniforms} /></Canvas></div>;
}
